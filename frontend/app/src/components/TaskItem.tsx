  ...
  function subscribeToTaskUpdates(taskId: string): EventSource {
    ...
    eventSource.addEventListener("complete", (event: Event) => {
      try {
        ...
        console.log(
          `Complete event received for task ${taskId}:`,
          messageEvent,
        );
        ...
        console.error(
          `Error handling complete event for task ${taskId}:`,
          error,
        );
      }
    });
    ...
    eventSource.onmessage = (event: MessageEvent) => {
      try {
        ...
        if (
          ...
          // Close connection on completion
          console.log(
            `Closing EventSource after completion (from onmessage) for task ${taskId}`,
          );
          eventSource.close();
        } else if (
          ...
          console.log(
            `Closing EventSource after error (from onmessage) for task ${taskId}`,
          );
          eventSource.close();
        } else if (
        ...
        // Close connection on error
        console.log(
          `Closing EventSource after message error for task ${taskId}`,
        );
        eventSource.close();
      }
    };
    ...
    eventSource.onerror = (error) => {
      ...
      if (["pending", "started"].includes(task.status.toLowerCase())) {
        ...
        const API_BASE_URL = "http://localhost:8000/api";
        console.log(
          `TaskItem: Attempting to fetch task status as fallback for ${taskId}`,
        );
        ...
        fetch(`${API_BASE_URL}/task/${taskId}`)
          ...
          .catch((fetchError) => {
            console.error(
              `TaskItem: Error fetching fallback status for ${taskId}:`,
              fetchError,
            );
    ...
    <div className="border rounded-lg p-4">
      <div className="flex justify-between items-start mb-2">
        ...
        <div className="flex flex-col items-end">
          <span
            className={`font-semibold text-base ${getStatusColor(task.status)}`}
          >
            {task.status}
          </span>
          <button
